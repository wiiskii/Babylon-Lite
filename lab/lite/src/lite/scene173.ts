// Scene 173: Navigation with toggleable box obstacle (port of playground #DPDNVH#4).
//
// Same starting state as scene 172 (10x10 ground, cylinder obstacle at (1.5,0,-1.5),
// box obstacle at (-2,1,1), single agent at (-3,0,3.5) computing path to (3,0,-3.5)).
//
// Unlike scene 172, the box obstacle is *toggled off* exactly 1 second after the
// ready frame: the box mesh is disposed and the navmesh re-baked. With `?freeze=1`,
// the toggle is never scheduled — parity reference is the pre-toggle state.
...
async function main(): Promise<void> {
    ...
    addCylinderObstacle(nav, { x: 1.5, y: 0, z: -1.5 }, 1, 0.5);
    let boxObstacle: ObstacleHandle | null = addBoxObstacle(nav, BOX_POS, BOX_HALF, BOX_ANGLE);
    ...
    type WireMesh = ReturnType<typeof createTube>;
