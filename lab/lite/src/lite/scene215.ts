// Scene 215: Cascaded Shadow Maps (CSM) on PBR receivers — torus-knot field.
//
// PBR counterpart of scene214 (playground #KY0N7T#84): a large PBR ground
// receiver under a field of 200 green PBR torus-knot casters scattered across a
// 2000-unit area, lit by a single DirectionalLight with a 4-cascade
// CascadedShadowGenerator (PCF5). A non-caster "Base" knot sits at the origin.
// This scene proves CSM works on PBR-material receivers (not just Standard).
//
// All caster transforms come from a seeded mulberry32 PRNG (NOT Math.random)
// drawn in the SAME order as the BJS oracle, and rotations are applied as
// quaternions computed with Babylon's YawPitchRoll convention, so the Lite
// render and the BJS golden match pixel-for-pixel. The directional light
// auto-fits the cascade Z bounds to the caster AABB (BJS
// autoCalcShadowZBounds = true; depthClamp = false).
...
async function main(): Promise<void> {
    ...
    // PBR ground receiver: light-gray dielectric (occ=1, rough=0.9, metal=0).
    const ground = createGround(engine, { width: SCENE_SIZE, height: SCENE_SIZE });
    ground.receiveShadows = true;
    ground.material = createPbrMaterial({
        baseColorTexture: createSolidTexture2D(engine, 0.6, 0.6, 0.6, 1),
        ormTexture: createSolidTexture2D(engine, 1.0, 0.9, 0.0, 1),
        usePhysicalLightFalloff: false,
    });

    // Shared green PBR caster material (occ=1, rough=0.5, metal=0.1).
    const knotMat = createPbrMaterial({
        baseColorTexture: createSolidTexture2D(engine, 0.1, 0.7, 0.15, 1),
        ormTexture: createSolidTexture2D(engine, 1.0, 0.5, 0.1, 1),
        usePhysicalLightFalloff: false,
    });
