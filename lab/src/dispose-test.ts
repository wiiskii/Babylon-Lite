...
async function createAndRender() {
    ...
    (engine as EngineContextInternal).device.pushErrorScope("validation");
    ...
    const preError = await (engine as EngineContextInternal).device.popErrorScope();
    if (preError) {
...
async function run() {
    try {
        ...
        (engine2 as EngineContextInternal).device.pushErrorScope("validation");
