...
export function createStdShadowFragment(shadowLights: ShadowLightSlot[]): ShaderFragment {
    ...
    // Fragment: compute shadow factors per-light (AD slot replaces default 1.0 values)
    // Note: shadowFactors array is declared in the template, we just overwrite entries here

    for (const slot of shadowLights) {
        const li = slot.lightIndex;
        const suffix = `_${li}`;

        // Per-light varying for projected position from this light
        varyings.push({ name: `vPosFromLight${suffix}`, type: "vec4<f32>" }, { name: `vDepthMetric${suffix}`, type: "f32" });

        // Per-light bindings (group "shadow")
        ...
        // Vertex: project world pos into this light's space
        vertexLines.push(
        ...
        // Fragment: compute shadow factor for this light
        if (slot.shadowType === "pcf") {
            fragmentLines.push(
...
return mix(darkness, 1.0, sh);
            ...
            // Per-light ESM sampling function
            helperParts.push(`
