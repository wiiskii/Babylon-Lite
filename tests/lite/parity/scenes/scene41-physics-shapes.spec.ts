/**
 * Scene 41 — Physics V2 shape debug viewer parity test.
 *
...
const sceneConfig = getSceneConfig(41);
const REFERENCE_DIR = path.resolve(__dirname, "../../../../reference/lite/scene41-physics-shapes");
const GOLDEN_REF = path.join(REFERENCE_DIR, "babylon-ref-golden.png");
const CAPTURE_FRAME = 10;
const CAPTURE_QUERY = `?captureFrame=${CAPTURE_FRAME}`;

test.skip(!!sceneConfig.skipParity, "Scene 41 skipped via skipParity in scene-config.json");
...
test("Scene 41 — Physics shape viewer matches Babylon.js reference", async ({ page }, testInfo) => {
    const browser = page.context().browser()!;
    const referencePath = await captureBjsReference(browser);

    await page.goto(`/scene41.html${CAPTURE_QUERY}`);
    await waitForCanvasReady(page, { timeout: 50_000, label: "Scene 41 Lite" });
    await waitForCanvasReady(page, { timeout: 50_000, label: `Scene 41 Lite at frame ${CAPTURE_FRAME}`, flag: "captureReady", pollMs: 100 });
