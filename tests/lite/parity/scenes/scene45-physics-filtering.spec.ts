/**
 * Scene 45 — Physics collision filtering parity test.
 *
...
const sceneConfig = getSceneConfig(45);
const REFERENCE_DIR = path.resolve(__dirname, "../../../../reference/lite/scene45-physics-filtering");
const GOLDEN_REF = path.join(REFERENCE_DIR, "babylon-ref-golden.png");
const CAPTURE_AFTER_SECONDS = 3;
const CAPTURE_QUERY = `?captureAfter=${CAPTURE_AFTER_SECONDS}`;

test.skip(!!sceneConfig.skipParity, "Scene 45 skipped via skipParity in scene-config.json");
...
test("Scene 45 — Physics collision filtering matches Babylon.js reference", async ({ page }, testInfo) => {
    const browser = page.context().browser()!;
    const referencePath = await captureBjsReference(browser);

    await page.goto(`/scene45.html${CAPTURE_QUERY}`);
    await waitForCanvasReady(page, { timeout: 50_000, label: "Scene 45 Lite" });
    await waitForCanvasReady(page, { timeout: 50_000, label: `Scene 45 Lite after ${CAPTURE_AFTER_SECONDS}s`, flag: "captureReady", pollMs: 100 });
